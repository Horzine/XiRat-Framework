...
namespace Xi.Extension.Collection
{
        ...
        where TDictionary : IDictionary<TKey, int>, new()
    {
        ...
        private readonly TDictionary indexInList;
        ...
        /// </summary>
        /// <exception cref="ArgumentNullException"></exception>
        public bool ContainsKey(TKey key) => indexInList.ContainsKey(key);
        ...
        private int CompareResult(int index1, int index2) => comparer.Compare(values[index1].Value, values[index2].Value);

        private void Exchange(int index, int max)
        {
            (values[max], values[index]) = (values[index], values[max]);
        ...
        {
            if (indexInList.TryGetValue(key, out int index))
            {
