...
namespace Xi.Extension.Collection
{
    public class MinHeap<T> : IEnumerable<T>
    {
        ...
        private int CompareResult(int index1, int index2) => comparer.Compare(values[index1], values[index2]);

        private void Exchange(int index, int max) => (values[max], values[index]) = (values[index], values[max]);
